...
import { CellVisited } from "./cell-visited";
...
export class Wumpus {
    ...
    move() {
        ...
        } else if (this.worldData[this.x][this.y].isWumpus && !this.isWumpusDead) {
            this.point -= 10000;
            this.gameOver = true;
        ...
        } else if (!this.isWumpusDead && this.areWeNearOfWumpus()) {
            console.log("Shoot");
            this.isShooting = true;
            this.isWumpusDead = true;
            return this.shootDirection;

            // Are we in pit loop?
        } else if (this.areWeInPitLoop()) {
            console.log("pit loop");
            if (this.x != 10 && this.worldVisited[this.x + 1][this.y].pitChance < 60) {
                this.randomMoves = 0;
                return this.Moves.Right;
            } else if (this.y != 10 && this.worldVisited[this.x][this.y + 1].pitChance < 60) {
                this.randomMoves = 0;
    ...
    areWeInDangerSpace() {
        ...
        if (this.worldData[this.x][this.y].isBreeze || (this.worldData[this.x][this.y].isStench && !this.isWumpusDead))
            return true;
        else
            return false;
    }
    ...
    CalcBreezeAndStench() {
        ...
        if (!this.worldVisited[this.x][this.y].nearDanger) {
            ...
            if (this.worldData[this.x][this.y].isStench && !this.isWumpusDead) {
                this.PitWumpusPercentage(false, true);
            }
        }
    }

    PitWumpusPercentage(pit:any, wumpus:any) {
    ...
    drawCell() {
        for (var i = 1; i <= 10; i++) {
            for (var j = 1; j <= 10; j++) {
                ...
                var img = "";
                if (cell.isPlayer && this.isShooting) {
                ...
                else if (cell.isWumpus && cell.isBreeze && this.isWumpusDead)
                    img = "wumpus_dead.png";
                else if (cell.isWumpus && cell.isBreeze)
                    img = "wumpus.png";
                else if (cell.isWumpus && this.isWumpusDead)
                    img = "wumpus_dead.png";
                ...
                $(".pboard .cell" + cell.x + "" + cell.y).css("background", "url(./assets/" + img + ") no-repeat #efefef");

                if (cell.isPlayer)
                    $(".mboard .cell" + cell.x + "" + cell.y).css("background", "url(./assets/player.png) no-repeat #d0d0d0");
